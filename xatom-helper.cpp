...
XAtomHelper::XAtomHelper(QObject *parent) : QObject(parent)
{
    ...
    m_titlebarLeftAtom = XInternAtom(QX11Info::display(), "_KWIN_UKUI_DECORATION_TITLEBARLEFT", false);
}
...
void XAtomHelper::setDecorationTitlebarLeft(int winId, int width)
{
    if(m_titlebarLeftAtom == None)
        return;

    XChangeProperty(QX11Info::display(), winId, m_titlebarLeftAtom, XA_CARDINAL,
                    32, XCB_PROP_MODE_REPLACE, (const unsigned char *) &width, sizeof(width));
}
